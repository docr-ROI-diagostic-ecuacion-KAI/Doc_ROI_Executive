(function () {
  ...
  const modelByKind = { trainers: "BM_FORMADOR", team: "BM_INCOMPANY", trainerTraining: "BM_FORMADOR_FORMADORES" };
  const formTypeByKind = { trainers: "trainers", team: "in_company", trainerTraining: "train_the_trainers" };
  ...
  function contactName(data) { return data.trainer_full_name || data.team_full_name || data.trainer_training_full_name || data.full_name || ""; }
  function role(data) { return data.trainer_role || data.team_role || data.trainer_training_role || ""; }
  function company(data) { return data.company || data.institution_or_company || data.organization || ""; }
  function needSummary(data) { return [data.discipline_subject_module, data.program_master_course, data.main_need, data.interest_format, data.topic_to_land, data.support_type, data.target_audience].filter(Boolean).join(" | "); }
  ...
  async function sendLead(form) {
    ...
    if (window.DocROITrack) window.DocROITrack("form_submit_success", { keyword: "Executive", keyword_id: "KW_EXECUTIVE", area: kind, form_type: payload.form_type });
    try { await fetch(leadEndpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload), keepalive: true }); }
    catch (error) { console.warn("Doc ROI lead bridge could not send lead", error); }
  }

  document.addEventListener("submit", function (event) { const form = event.target.closest("form[data-form-kind]"); if (form) sendLead(form); }, true);
  document.addEventListener("click", function (event) { const button = event.target.closest("[data-open-form], [data-target], a[href]"); if (!button || !window.DocROITrack) return; window.DocROITrack("cta_click", { keyword: "Executive", keyword_id: "KW_EXECUTIVE", area: button.dataset.openForm || button.dataset.target || "executive", link_text: (button.textContent || "").trim().slice(0, 180) }); });
})();
